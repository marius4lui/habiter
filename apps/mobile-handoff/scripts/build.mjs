...
const fallbackAppleAppId = "0000000000.com.example.habiter";
const appleAppId = process.env.HABITER_APPLE_APP_ID ?? fallbackAppleAppId;
if (!/^[A-Z0-9]{10}\.[A-Za-z0-9.-]+$/.test(appleAppId)) fail("HABITER_APPLE_APP_ID must be a 10-character Team ID plus bundle ID.");
if (production && appleAppId === fallbackAppleAppId) fail("Production build requires the signed app's real HABITER_APPLE_APP_ID.");
...
console.log(`Built stateless mobile handoff${production ? " for production" : " with the documented local Apple placeholder"}.`);
