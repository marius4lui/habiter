...
Linux and Windows keep the manifest at the installation root. macOS keeps it in the adjacent `<canonical Habiter.app>.habiter-install.json` file so ownership evidence does not mutate or invalidate the signed bundle. The uninstaller still recognizes the earlier internal marker as a legacy compatibility path, but rejects ambiguous internal-plus-external evidence. A manifest is evidence, never a deletion instruction: uninstallers recanonicalize every value, reject broad roots and link/reparse escapes, verify executable/bundle identity and each integration target, and preserve mismatches. Pre-manifest installs require multiple matching legacy signals and an additional warning.
...
Android is built in two flavors with the same application ID. `direct` produces a signed universal APK with the installer permission and FileProvider; `store` produces an AAB without either direct-install capability and alone links Google Play's official app-update library. The build and release workflows compile both flavor-native test suites, inspect the merged manifests with `scripts/android/verify-update-flavors.sh`, and verify both release artifacts against `ANDROID_CERT_SHA256`. The build flavor is authoritative, while the detected Android install source is an additional safeguard: a Store-installed build never receives a direct APK. Play controls staged rollout, device/SDK eligibility, download, and installation.

Desktop release artifacts declare explicit `format` and `primary` fields. Linux's primary artifact is the AppImage; its supplemental tar bundle is never selected for self-replacement. Windows and macOS use one primary ZIP. The client downloads into a bounded per-user cache, rejects non-HTTPS URLs and redirects, resumes partial transfers when the server honors Range, and verifies exact size plus SHA-256 before promotion.

In-app desktop installation remains gated twice. The manifest must mark Windows/macOS artifacts signed, and the helper must independently establish platform trust immediately before replacement. Windows requires valid Authenticode on current and next executable with the same signer certificate. macOS requires exact bundle ID, strict code-sign verification, Gatekeeper assessment, and the same non-ad-hoc Team Identifier. Only exact maintained user-scoped roots are eligible. Current unsigned Windows and macOS artifacts therefore open the maintained external installer; system-wide and package-manager installations always remain external and never trigger elevation.

Linux AppImage, signed Windows, and signed macOS helpers rehash the payload after the app exits, stage beside the exact target, keep an adjacent backup, relaunch, and remove the backup only after the new process survives its startup probe. Early exit restores the backup and writes the stable `install_failed` marker. Platform-build CI runs the helper contract test on Linux, macOS, and Windows; the Windows runner parses the generated PowerShell helper and POSIX runners syntax-check the generated shell helpers.
...
## Update kill switch and client rollback

If a published update must stop being offered, do not delete its tag or artifacts and do not replace bytes at an existing URL. First roll the production Worker back to the last known-good signed envelope, verify `/api/v1/manifest` and `/api/v1/update` select the previous build, and pause publication. Then land a reviewed source-manifest correction that returns the affected entry to `draft` with `publishedAt: null`, generate and sign the production envelope through normal tooling so drafts are omitted, and redeploy. Preserve the original source commit, tag, checksums, incident record, and GitHub assets as forensic history; a corrected release uses a new version and build number.

On the next successful check, clients compare any persisted download with the newly selected build and complete artifact identity. A withdrawn or replaced candidate is deleted from the native/per-user cache and cannot be launched through the Update Center. A client that is offline cannot receive a remote kill switch; if the artifact has already installed, ship a higher corrective release and use the platform rollback/recovery process. Test each kill switch with a clean client, a partially downloaded client, and a verified-ready client before declaring recovery.

For a bad installed desktop update, retain the helper rollback evidence and current/backup paths, reproduce with the exact release artifact, and publish a higher fixed build. Never repoint the same version, bypass signature policy, or instruct users to disable Gatekeeper, SmartScreen, Android installer protections, or their package manager.
