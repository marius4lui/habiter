...
export const sqliteMigrations: readonly SqliteMigration[] = [
  {
    ...
    sql: `
      ...
      CREATE TABLE sync_entities (
        ...
        lifecycle_sequence INTEGER,
        lifecycle_device_id TEXT,
      ...
      CREATE TABLE sync_device_watermarks (
      ...
      CREATE TABLE auth_one_time (
        kind TEXT NOT NULL CHECK (kind IN ('login_challenge', 'authorization_code')),
  ...
  {
    version: 2,
    name: "cursor_and_auth_indexes",
    sql: `
      CREATE INDEX sync_operations_entity_cursor
        ON sync_operations(entity_id, server_cursor);
      CREATE INDEX sync_operations_device_sequence
        ON sync_operations(device_id, device_sequence);
      CREATE INDEX auth_one_time_expiry
        ON auth_one_time(kind, expires_at);
      CREATE INDEX auth_sessions_device
        ON auth_sessions(device_id, revoked_at);
      CREATE INDEX auth_sessions_family
        ON auth_sessions(family_id, revoked_at);
    `
  }
] as const;

export const currentSqliteSchemaVersion = sqliteMigrations.at(-1)!.version;
