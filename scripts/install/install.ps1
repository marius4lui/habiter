...
function Install-DesktopIntegration([string]$Root) {
    $startMenu = Join-Path ([Environment]::GetFolderPath('Programs')) 'Habiter.lnk'
...
function Invoke-HabiterInstall {
    ...
    if ($DryRun) {
        ...
        $script:TempDirectory = Join-Path ([IO.Path]::GetTempPath()) ("habiter-" + [guid]::NewGuid().ToString('N'))
        ...
        if ($actual -ne $release.artifact.sha256) { throw 'SHA-256 mismatch. Installation aborted.' }
