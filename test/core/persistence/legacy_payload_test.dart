...
void main() {
  ...
  group('StorageService legacy compatibility', () {
    ...
    test('loads all existing v0 keys without changing their meaning', () async {
      ...
      expect(habits.map((habit) => habit.id),
          <String>['legacy-daily-water', 'legacy-custom-reading']);
    ...
    test('updates schedule and reminder fields without losing other data',
        () async {
      await StorageService.updateHabit(
          'legacy-custom-reading', <String, dynamic>{
        'customDays': <int>[2, 4, 6],
        'notificationEnabled': true,
        'notificationTime': '07:05',
      });

      final updated = (await StorageService.getHabits())
          .singleWhere((habit) => habit.id == 'legacy-custom-reading');
    ...
    test('legacy preference and app-lock JSON remain serializable', () async {
      ...
      expect(jsonDecode(jsonEncode(preferences.toMap())),
